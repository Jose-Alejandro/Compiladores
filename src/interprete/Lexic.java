...
public class Lexic {
    ...
    public boolean AnalizarLexema(String inCadena, ArrayList<String> alfabeto, ArrayList<ArrayList<Integer>> tabla){
        char cadena[];
        Lexema = inCadena;
        cadena = Lexema.toCharArray();
        
        int i, j, k = -1, longitudLexema; //i=Carater actual que se esta analizando.
        //int longitudColumna = tabla[EdoActualLexema].length - 1;
        int longitudColumna = tabla.get(0).size() - 1;
        longitudLexema = Lexema.length();
        
        for (i = 0; i < longitudLexema; i++) //Revisara toda la cadena de entrada
        {
            k = -1;
            loop:for (j = 0; j < alfabeto.size(); j++) //Buscamos el indice del alfabeto donde esta el caracter de la cadena de entrada.
            {
                if ( (cadena[i] + "").equals(alfabeto.get(j))) //Si el carcter de entrada es igual al del alfabeto
                {
                    k = j; //Guardamos el valor del indice.
                    break loop;
                }
            }
            if(k == -1){    //No se encontró el caracter de entrada en el alfabeto
                System.out.println("La cadena no es Valida.");
                //pilaToken.clear();
                return false; //El Lexema de entrada es incorrecto "Cadena NO valida".
            }
            
            if (tabla.get(EdoActualLexema).get(k) != -1) //Revisamos si hay transición del estado actual a algun otro.
            {
                IndInicioLexema = i; //Aqui empieza el lexema que si es reconocido por el AFD
                EdoActualLexema = (tabla.get(EdoActualLexema).get(k)); //El estado actual en el que nos encontramos cambia a donde hay transición.
                if (tabla.get(EdoActualLexema).get(longitudColumna) != -1 ) //Revisamos si es estado de aceptación
                {
                    EdoAcept_Previo = true; //Existe un estado de aceptacion previo
                    IndCaracterEdoAcept = i;
                    Token = (tabla.get(EdoActualLexema).get(longitudColumna)); //Nos vamos a la ultima columna
                } 
            }
    ...
    public boolean AnalizarLexema(String inCadena, char[] inAlf) {
    ...
    public String ImprimeTokens(String output) {
        //String output = "";
        Stack<String> pilaTokencopia;
        pilaTokencopia = pilaToken;
        while (!pilaTokencopia.empty()) {
            output += (pilaTokencopia.pop() + ", ");
        }
        return output;
    }
}
